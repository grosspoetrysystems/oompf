...
async function freshRepository(): Promise<{
  ...
  const ddl = await Bun.file(
    new URL("../packages/database/migrations/0001_profiles.sql", import.meta.url),
  ).text();
  await client.exec(ddl);
  const db = drizzle(client, { schema }) as unknown as ProfileDatabase;
  return { repository: createProfileRepository(db), close: () => client.close() };
}
...
function assertSuccess(code: number | undefined, output: string, label: string): void {
...
export async function runLocalSmoke(): Promise<SmokeSummary> {
  ...
  try {
    const store = memoryFs({ "/omp/profiles/work/agent/config.yml": PROFILE_YAML });
    const fetchGist = async () => resolvedGist();

  const httpFetch = async (url: string, init?: { method?: string; body?: string }) => {
    ...
    if (parsed.pathname === "/api/profiles" && method === "POST") {
      ...
      const record = await indexPublicGist(
        { source: String(body.source ?? ""), ompVersion: body.ompVersion },
        { repository, fetchGist },
      );
  ...
  const deps = {
    ...
    resolveInstallTarget: async (name: string) => `/omp/profiles/${name}/agent`,
  };
  ...
  const publishResult = JSON.parse(published.out) as { oompfUrl: string; addCommand: string };
  assert.ok(publishResult.oompfUrl.startsWith(`${BASE_URL}/p/prof_`));
  assert.equal(publishResult.addCommand, `oompf add ${publishResult.oompfUrl}`);
  ...
  const inspected = await runCli(deps, ["inspect", publishResult.oompfUrl, "--json"]);
  assertSuccess(inspected.code, inspected.out, "inspect");
  assertNoRawArtifact(inspected.out, "inspect output");
  const inspectResult = JSON.parse(inspected.out) as { sourceType: string; models: string[] };
  ...
  const searchResult = JSON.parse(searched.out) as { count: number; results: Array<{ id: string }> };
  assert.equal(searchResult.count, 1);
  assert.equal(searchResult.results[0]?.id, id);

  const added = await runCli(deps, ["add", publishResult.oompfUrl, "--name", "smoke-work", "--json"]);
  assertSuccess(added.code, added.out, "add");
  assertNoRawArtifact(added.out, "add output");
  const addResult = JSON.parse(added.out) as { path: string; command: string };
  ...
  const collision = await runCli(deps, ["add", publishResult.oompfUrl, "--name", "smoke-work"]);
  assert.ok((collision.code ?? 0) > 0, "second add should fail on collision");
  assert.equal(store.writes.length, writesAfterInstall, "collision must not write again");
